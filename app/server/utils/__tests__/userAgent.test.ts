...
describe('resolveTarkovTrackerUserAgent', () => {
  it('names this instance when APP_URL is configured', () => {
    expect(resolveTarkovTrackerUserAgent({ APP_URL: 'https://tracker.tarkov-stammtisch.de' })).toBe(
      'TarkovTracker/1.0 (+https://tracker.tarkov-stammtisch.de)'
    );
  });
  ...
  it('normalizes a bare hostname (no scheme) into an https origin', () => {
    expect(resolveTarkovTrackerUserAgent({ APP_URL: 'tracker.tarkov-stammtisch.de' })).toBe(
      'TarkovTracker/1.0 (+https://tracker.tarkov-stammtisch.de)'
    );
  });
  it('drops a configured path/query/hash, keeping only the origin', () => {
    expect(
      resolveTarkovTrackerUserAgent({
        APP_URL: 'https://tracker.tarkov-stammtisch.de/some/path?x=1#h',
      })
    ).toBe('TarkovTracker/1.0 (+https://tracker.tarkov-stammtisch.de)');
  });
  ...
  it('is not vulnerable to header-injection via whitespace/CRLF in the app url', () => {
    const result = resolveTarkovTrackerUserAgent({
      APP_URL: 'https://tracker.tarkov-stammtisch.de\r\nX-Injected: 1',
    });
